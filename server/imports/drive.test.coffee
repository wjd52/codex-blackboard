...
EVERYONE_PERM =
  ...
  role: 'writer'
  type: 'anyone'
defaultPerms =  [GIVEN_OWNER_PERM, EVERYONE_PERM]
...
describe 'drive', ->
  ...
  it 'creates folder', ->
    gapi.expects('exec').withArgs api.children, 'list', sinon.match
      folderId: 'root'
      q: 'title=\'MIT Mystery Hunt 2014\''
      maxResults: 1
    .returns items: []
    gapi.expects('exec').withArgs api.files, 'insert', sinon.match
      resource:
        title: 'MIT Mystery Hunt 2014'
  ...
  describe 'when folders already exist', ->
    drive = null
    beforeEach ->
      gapi.expects('exec').withArgs api.children, 'list', sinon.match
        folderId: 'root'
    ...
    describe 'findPuzzle', ->
      ...
      it 'returns spreadsheet', ->
        ...
        chai.assert.deepEqual drive.findPuzzle('New Puzzle'),
          id: 'newpuzzle'
          spreadId: 'newsheet'
